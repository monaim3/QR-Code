...
import { updateSocialChannel } from "@/store/slices/businessSlice";
...
export default function SocialInputCard({ channelId, handleDelete }: Props) {
  ...
  const socialChannel = business.socialChannels.find((ch) => ch.id === channel.id);
    ...
    >
      ...
      <div className="flex items-start gap-2 flex-1 w-full desktop:w-auto">
        <div className="flex flex-col gap-1 flex-1">
          <input
            type="url"
            placeholder={t("generator__content_form_section__social__input_placeholder")}
            value={url}
