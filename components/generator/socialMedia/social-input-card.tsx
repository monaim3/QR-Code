...
export default function SocialInputCard({ channel, handleDelete }: Props) {
  ...
  const urlError = showErrors
    ? validationErrors[`socialUrl_${channel.id}`]
    : undefined;
  ...
  if (channel.isIcon) {
    ...
    channels = social.socialChannels.find((ch) => ch.id === channel.id);
  } else {
    channels = channel;
  ...
  const handleDescriptionChange = (value: string) => {
    dispatch(
      updateSocialChannel({ id: channel.id, changes: { description: value } }),
    );
  };
  const t = useT();
  return (
