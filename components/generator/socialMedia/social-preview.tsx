  ...
  return (
    <ScrollArea className="w-full h-full rounded-[32px]">
      ...
      <div className="w-full h-full flex flex-col items-center gap-6 pb-8 pt-[66.46px] px-5 relative">
        <div
          className="absolute left-0 top-0 h-[145px] w-full z-[1] rounded-[32px]"
        ...
        <div className="absolute left-0 right-0 top-[32px] flex flex-col items-center justify-center px-4 z-[2]">
          <Carousel
            images={social.isDefault ? defaultImages : social.carousels}
          />
          <p className="text-[18px] leading-[26px] font-bold text-[var(--Black)]">
            {social.isDefault ? "Scan & Connect" : social.socialInfo.headLine}
          </p>
          <p className="text-[10px] leading-[16px] font-regular text-[var(--Black)] text-center">
            {social.isDefault
              ...
              : social.socialInfo.description}
          </p>
          <div className="w-full pt-6">
            {(social.isDefault ? defaultChannels : social?.socialChannels).map(
              (channel, index) => {
                const findChannel = socialChannels.find(
                  (ch) => ch.id === channel.id,
                );
                ...
                return (
                  ...
                  >
                    {channel.isIcon && ChannelIcon ? (
                      <ChannelIcon />
                    ) : (
                      <img
                        src={channel.icon ?? ""}
                        alt={channel?.name}
                        className="w-[33.33px] h-[33.33px] object-contain"
                      />
                    )}
                    <div className="flex flex-col flex-1 items-start justify-start">
                      <p
                        className={`text-[14px] leading-[22px] font-medium ${buttonTextColor()} text-[var(--Black)]`}
                      >
                        {channel.name}
                      </p>
                      <p
                        className={`text-[12px] leading-[20px] font-regular ${buttonTextColor()} text-[var(--Grey)]`}
                      >
                        {channel.description}
                      </p>
                    </div>
