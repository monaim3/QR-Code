...
export default function AppPreView() {
  ...
  return (
    <ScrollArea className="w-full h-full relative">
      ...
      <div
        className="absolute inset-0 w-full h-[96px] rounded-[32px]"
      ...
      <div className="flex flex-col items-center mt-[150px] px-[20px]">
        <p
          className={`text-[18px] leading-[26px] text-[var(--Black)] font-bold`}
        >
          {app.appDefaultState ? "MindFlow" : app.appInfo.appName}
        </p>
        <p
          className={`text-[10px] leading-[16px] text-[var(--Black)] font-regular`}
        >
          {app.appDefaultState ? "CreativeCore Apps" : app.appInfo.developer}
        </p>
        <p
          className={`text-[10px] leading-[16px] text-[var(--Black)] font-regular mt-[10px] text-center`}
        >
          {app.appDefaultState
        ...
        <div className="w-full">
          {app.appInfo.buttons.map((button, index) => {
            return (
              ...
              >
                <p
                  className={`text-[12px] leading-[20px] ${buttonTextColor()}`}
                >
                  {button.text}
                </p>
              </div>
        ...
        <div className="w-full pt-4">
          {(app.appDefaultState ? app.appLinks : app.appStoreLinks).map(
            (button, index) => {
              return (
                <div
                  key={index + 1}
                  className="h-[40px] bg-[var(--Black)] rounded-[10px] flex items-center justify-center mb-2"
                >
                  {button.storeName === "appStore" ? (
                    <div
                      className="h-[40px] w-full bg-[var(--Black)] rounded-[10px] flex items-center justify-center"
