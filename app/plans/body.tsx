...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  return (
    <div
      key={plan.id}
      className={`${plan.popular ? "h-full outline outline-2 outline-[var(--Blue)]" : "h-full"} flex flex-col bg-white shadow-card rounded-[12px] hover:shadow-xl transition-shadow duration-300 desktop:w-[384px]`}
    >
      {plan.popular && (
        <div className="h-[42px] rounded-t-[12px] bg-[var(--Blue)] w-full flex items-center justify-between px-4 desktop:px-8">
          <p className="text-[16px] leading-[26px] text-white font-bold">
            Most Popular
          </p>
      ...
      <div className="p-4 desktop:p-8">
        {index === 0 && <div className="hidden desktop:block h-[44px]" />}
      ...
      "
        ...
        <div
          className={`hidden desktop:flex rounded-[10px] items-center justify-center h-[48px] w-full ${plan.popular ? "bg-[var(--Blue)] hover:bg-[var(--Blue-hover)]" : "outline outline-1 outline-[var(--Blue)]"} transition duration-300 my-[24px]`}
        >
          <p
            className={`text-[18px] leading-[26px] font-semibold ${plan.popular ? "text-white" : "text-[var(--Blue)]"}`}
          >
            Choose plan
          </p>
        </div>
        <ul className="hidden desktop:block space-y-[12px]">
          {plan.features.map((feature, index) => (
            ...
            >
              <CheckIcon
                fill={`${plan.popular ? "var(--Blue)" : "var-(--Dark-grey)"}`}
              />
      ...
      <div className="max-w-full">
        <Container>
          <div className="flex flex-col pt-[40px] desktop:pt-[80px] desktop:pb-[160px]">
            ...
            <div className="hidden desktop:flex items-center justify-center mt-4">
              <CurrenctSelector textClass="text-[var(--Grey)]" />
            </div>

            {/* Pricing Cards - Wrapped in its own flex container */}
            <div className="pt-[24px] desktop:pt-[40px]">
              <div className="flex items-end desktop:h-[586px]">
                <div className="max-w-[800px] w-full mx-auto grid grid-cols-1 md:grid-cols-2 gap-[16px] desktop:gap-[24px]">
